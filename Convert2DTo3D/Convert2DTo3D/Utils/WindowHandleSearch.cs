...
namespace CreateRack.DetailCommands.Utils
{
    public class WindowHandleSearch : IWin32Window, System.Windows.Forms.IWin32Window
    {
        ...
        public WindowHandleSearch(HWND hwnd)
        {
            // Assert valid window handle
            Debug.Assert(HWND.Zero != hwnd, "Null window handle");
        ...
        private static extern uint GetWindowThreadProcessId(HWND hWnd, out uint processId);
        ...
        /// <returns></returns>
        public static HWND GetMainWindow(int pid)
        {
            HWND shellWindow = GetShellWindow();
            List<HWND> windowsForPid = new List<HWND>();
        ...
        /// <returns></returns>
        private static HWND DetermineMainWindow(List<HWND> handels)
        {
        ...
        /// <summary>
        /// Activates a window from its handle, restoring it first if minimized
        /// </summary>
        /// <param name="hwnd">Handle of the window to activate</param>
        /// <returns>True if successful, false otherwise</returns>
        public static bool ActivateWindowFromHandle(IntPtr hwnd)
        {
            ...
            // Set the window to foreground (active)
            return SetForegroundWindow(hwnd);
        }

        /// <summary>
        /// Activates the main Revit window
        /// </summary>
        /// <returns>True if successful, false otherwise</returns>
        public bool ActivateMainWindow()
        {
            try
            {
                ...
                HWND mainWindow = MainWindowHandle.Handle;

                // Check if we found a valid handle
                if (mainWindow == HWND.Zero)
                    return true;

                // Activate the main window
                return ActivateWindowFromHandle(mainWindow);
            }
            catch (Exception)
            {
            }
